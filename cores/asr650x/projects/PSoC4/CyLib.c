...
#if (CY_IP_SRSSLT)
        freq = (((uint32)((CY_SYS_CLK_IMO_SELECT_REG & ((uint32)CY_SYS_CLK_IMO_SELECT_FREQ_MASK)) << CY_SYS_CLK_IMO_SELECT_FREQ_SHIFT) + CY_SYS_CLK_IMO_MIN_FREQ_MHZ) >>
...
/* For the WCO locking mode, the IMO gain needs to be CY_SYS_CLK_IMO_TRIM4_GAIN */
        ...
        else
        {
            ...
            lfLimit = (lfLimit > CY_SYS_CLK_WCO_CONFIG_DPLL_LF_LIMIT_MAX) ? CY_SYS_CLK_WCO_CONFIG_DPLL_LF_LIMIT_MAX : lfLimit;
...
{
    return ((0u != (CY_SYS_CLK_WCO_CONFIG_REG & CY_SYS_CLK_WCO_CONFIG_DPLL_ENABLE)) ? (uint32)1u : (uint32)0u);
}
...
/* Set oscillator interface control port to USB */
    ...
    if (48u == freq)
    {
        CY_SYS_CLK_IMO_TRIM1_REG = (0u != CySysClkUsbCuSortTrim) ? (uint32)CY_SFLASH_CU_IMO_TRIM_USBMODE_48_REG : (uint32)CY_SFLASH_IMO_TRIM_USBMODE_48_REG;
    }
    else if (24u == freq)
    {
        CY_SYS_CLK_IMO_TRIM1_REG = (0u != CySysClkUsbCuSortTrim) ? (uint32)CY_SFLASH_CU_IMO_TRIM_USBMODE_24_REG : (uint32)CY_SFLASH_IMO_TRIM_USBMODE_24_REG;
    }
    else
    {
        /* Do nothing */
...
{
    return ((0u != (CY_SYS_CLK_USBDEVv2_CR1_REG & CY_SYS_CLK_USBDEVv2_CR1_ENABLE_LOCK)) ? (uint32)1u : (uint32)0u);
}
#endif /* (CY_IP_IMO_TRIMMABLE_BY_USB) */
...
#if (CY_IP_SRSSLT && CY_IP_PLL)
        sysclkSource |= (((uint32)(CY_SYS_ECO_CLK_SELECT_REG & CY_SYS_ECO_CLK_SELECT_ECO_PLL_MASK)) << CY_SYS_CLK_SELECT_HFCLK_PLL_SHIFT);
#endif /* (CY_IP_SRSSLT && CY_IP_PLL) */
    }
...
void CySysClkWriteSysclkDiv(uint32 divider)
{
    ...
    CY_SYS_CLK_SELECT_REG = ((uint32)(((uint32)divider & CY_SYS_CLK_SELECT_SYSCLK_DIV_MASK) << CY_SYS_CLK_SELECT_SYSCLK_DIV_SHIFT)) |
                            (CY_SYS_CLK_SELECT_REG & ((uint32)(~(uint32)(CY_SYS_CLK_SELECT_SYSCLK_DIV_MASK << CY_SYS_CLK_SELECT_SYSCLK_DIV_SHIFT))));
...
void CySysClkWriteHfclkDiv(uint32 divider)
{
    ...
    CY_SYS_CLK_SELECT_REG = ((CY_SYS_CLK_SELECT_REG & ((uint32)(~(CY_SYS_CLK_SELECT_HFCLK_DIV_MASK << CY_SYS_CLK_SELECT_HFCLK_DIV_SHIFT)))) |
